...
FactoryPushArduino = serial.Serial('COM4', 9600)
...
while True:
    ...
    print("Reading the input of the phototransistors")
    try:
    ...
    if (circleColor == 'WHITE'):
        ...
        timers.append([1, currentTime+25, f"W {whiteAngleDefault}\n"])
        # update the white position
        WhitePosition = 1
        # sleep to wait for the servo to move
        check_white = 0
        timers.append([2, currentTime+1,'W'])
        ...
        timers.append([1, currentTime+19, f"B {blackAngleDefault}\n"])
        ...
        print("MultiColor circle detected")

    # Read the data from the BlackWhiteArduino
    print("Checking the data from the catapult phototransistor")
    ...
    if bw_arduino_data.split(': ')[0] == 'S3':
        ...
        print(f'lightSens3Data: {defaultLightSens3}')
        ...
        if (lightSens3WasTaken == False):
        ...
        elif abs(lightSens3Data-defaultLightSens3) > 300 and not any("R 90\n" in s for s in timers):
            ...
            timers.append([1, currentTime+8, f"S 0\n"])
            timers.append([1, currentTime+9, "R 90\n"])
...
# Check if anything time related needs to end
    ...
    for index, timer in enumerate(timers):
        ...
        if (currentTime > float(timer[1])):
            ...
            if (timer[0] == 1):
                if 'W' in timer[2]:
                    print("Moving White servo to default position")
