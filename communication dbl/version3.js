...
// used to prevent informing robots when someone leaves during break time
let isItBreak = false;
...
let diskNumberGenerator = generateNumbers(1000);
...
/**
 * When a disk is processed, it is added to the map, the robots are informed, 
 */
async function handleProcessedDisk(nameRobot) {
  // Perform initial processing immediately
...
function breakTime(){
  isItBreak = true;
  // no robot is allowed to work
  for (const [robot, _] of robots) {
    client.publish(`robots/allowed/${robot}`, '0', { qos: 1 });
  }
  logger.logTime("Time for break. The robots will start working again after a minute.");
  console.log(`${diskNumberGenerator.next().value- 1} disks were processed in total.`)
  setTimeout(() => {
    // the number of processed disks goes to 0
    for (const [robot, _] of robots) {
      robots.set(robot, 0);
      
    }
    logger.logTime("The break ended. The counting of disks starts from 0 again.");
    // new disk generator
    diskNumberGenerator = generateNumbers(1000);
    // start work time and inform the robots
    isItBreak = false;
    informRobots();
    workTime();
...
// first working shift : start the work shift
myEmitter.once("firstWork", () => {
  logger.logTime("A robot already joined. First working shift starts.");
  workTime();
})
